...
- `setProceedOnNetworkFailure(proceed:)` is not a public method in this release. Configure a service mutator for an explicit per-status decision override (see [failure handling](README.md#failure-handling)).
...
See [initialization and failure handling](README.md#initializing-approov) and [testing](TESTING.md).
